...
def repo_slug():
    """回傳 "owner/name"。CI 上 GitHub Actions 會給 GITHUB_REPOSITORY;
    本機就問 git remote。fork 的人不必改這支腳本裡的任何一行。"""
    ...
    try:
        ...
        pass
    return 'yazelin/ai-comic-starter'
...
def load_canon():
    ...
    # 冷啟動:episodes.json 還是空的,這是第一話,本來就沒有前情提要。
    # 樣板一定會經過這個狀態,不能像連載中的作品那樣直接拒跑。
    if not ordered:
        return {'next_n': 1, 'episodes': [], 'rules': rules, 'recent': ''}
    # 但「已經列了話數卻找不到分鏡」是真的不一致,那要停下來:
    # 這條產線的價值就在連續性,寧可停也不要產一話接不上的東西。
    if not recent.strip():
        raise RuntimeError(
            f'episodes.json 列了 {len(ordered)} 話,卻讀不到任何一話的分鏡'
            '(story/epN.md),LLM 會完全沒有前情提要。'
...
def fetch_wishes():
    ...
    """
    try:
        query = _WISH_QUERY_TMPL % (REPO_OWNER, REPO_NAME)
...
# 班底一律從 cast.json 推導,不寫死任何一部作品的角色。
CHARS = frozenset(prompt._CAST['cast'])
...
# 常駐主角。有些作品會把某個角色(例如反派)排除在「主角群」之外,這個樣板
# 沒有那種區分,就是全體班底。真的需要區分時在 cast.json 加一個欄位,
# 不要寫死在程式裡。
MAIN_CAST = tuple(prompt._CAST['cast'])

# 群像格的說法。scene 這樣寫就代表全體都在畫面上,不必逐一點名。
GROUP_PHRASES = ('EVERYONE', 'BOTH OF THEM', 'THE WHOLE CAST', 'ALL CHARACTERS')

CHAR_TAGS = {
    slug: (prompt._CAST['cast'][slug]['desc'].split(' model sheet')[0].strip())
    for slug in prompt._CAST.get('cast', {})
}
...
def validate_plan(plan, next_n, titles, pages_expected=None):
    ...
    for pg in pages:
        ...
        # 英文,speaker 只是給人看的標籤,所以模型只好自己補一個,而且通常補錯人。
        ...
        for panel in pg.get('panels') or []:
            ...
            upper = scene.upper()
            # 群像格:GROUP_PHRASES 那幾種寫法代表全體都在畫面上,只是沒
            # 逐一點名。這是合法寫法,不該被擋。不在主角群裡的角色要另外
            # 點名。
            ...
            missing = sorted(
                ...
                and not (grouped and sp in MAIN_CAST)
            )
            if missing:
                errs.append(
...
# 預設就是 Google 官方端點。上游那條產線的預設值指向作者自架的私有中繼,
# 照抄的人不改就會失敗,而且錯誤訊息看起來像金鑰壞了;樣板不該有那種陷阱。
GEMINI_BASE = os.environ.get('GEMINI_WEB_BASE_URL') or 'https://generativelanguage.googleapis.com'
...
UA = f'{REPO_NAME}-pipeline/1 (+https://github.com/{REPO_SLUG})'
...
# 同上:預設走 Google 官方影像模型,只要一把 AI Studio 金鑰,不必自架東西。
IMAGE_PROVIDER = (os.environ.get('IMAGE_PROVIDER') or 'gemini').lower()
...
SPEAKER_REF = {k: k for k in CHARS}
...
CN = '一二三四五六七八九十'
NAME = {k: v['name'] for k, v in prompt._CAST['cast'].items()}
...
def _cell(s):
    ...
    把整列切成錯的欄數。對白裡出現 `|` 不是罕見情況(程式碼、表格、顏文字)。
...
def pr_body(plan, n, wishes, wish_err=None, branch=None, verdicts=None):
    ...
    out.append('**你要看的不是劇情，是圖有沒有照劇本畫。** '
               ...
               '分鏡完全正確、圖卻沒把那個效果畫出來，整頁的笑點就沒了，'
               '而讀分鏡檔案完全看不出來。\n')
    cover = ROOT / f'images/ep{n}/00-cover.webp'
    if cover.is_file():
        # 封面本來完全沒出現在 PR 裡,而它跟內頁一樣是生成的、一樣會出錯。
        out.append('\n### 封面\n')
        out.append(f'![封面](https://raw.githubusercontent.com/{REPO_SLUG}/'
...
def _wording_prompt():
    ...
    這段不能是 f-string(裡面有 JSON 範例的大括號),也不能用 % 佔位符
    (它本身就是拿去做 `% listing` 的模板),所以用一個不會跟兩者衝突的
    自訂記號。"""
    return WORDING_PROMPT.replace('<<CHARACTER_NAMES>>', '、'.join(NAME.values()))
...
def wording_problems(plan):
    ...
    **這是 validate_plan 抓不到的那一類。** validate_plan 管頁數、框型、簡繁、
    ...
    try:
        raw = call_llm(_wording_prompt() % listing)
...
def name_offpanel_speakers(plan):
    ...
    上去,那正是原本那個 bug:一話六頁裡四頁的台詞都被畫給了另一個角色。
        ...
        for panel in pg.get('panels') or []:
            ...
            for sp in dict.fromkeys(ln.get('speaker') for ln in (panel.get('lines') or [])):
                if sp not in CHAR_TAGS:
                    continue
                tag = CHAR_TAGS[sp]
                if tag in scene or sp.upper() in upper or (grouped and sp in MAIN_CAST):
                    continue
...
def verify_episode(n, plan):
    ...
    寫的」跟「圖畫出來的」之間那道縫:分鏡寫著某個效果、圖卻沒畫出來,
    讀分鏡檔案永遠看不出來。
