...
# 常駐主角。neko-tensei 那邊要把魔王排除在「四貓」之外,樣板沒有那種區分,
# 就是全體班底。真的需要區分時在 cast.json 加一個欄位,不要寫死在程式裡。
...
# 預設就是 Google 官方端點。neko-tensei 那邊預設指向作者的私有中繼,
# fork 的人不改就會失敗;樣板不該有那種陷阱,所以這裡翻轉預設值。
