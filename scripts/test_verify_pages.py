...
def _has_cases():
    import pathlib
    p = pathlib.Path(__file__).parent / 'verify_cases.txt'
    return any(l.strip() and not l.startswith('#') for l in p.read_text('utf-8').splitlines())
...
class TestCases(unittest.TestCase):
    ...
    @unittest.skipUnless(_has_cases(), '這個樣板還沒有累積回歸案例')
    def test_回歸集要有陽性也要有負控制(self):
...
class TestRules(unittest.TestCase):
    def test_讀得到規則正文(self):
        ...
        self.assertIn("格莉奇", rules)
        self.assertIn("黑洞先生", rules)
