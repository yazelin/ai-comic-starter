...
# service worker 的快取名前綴。寫死別人的縮寫會讓兩個站在同源下互相打架,
# 而 GitHub Pages 的專案站全部同源。
PREFIX = CFG['site'].get('cache_prefix') or ROOT.name

# GitHub Discussions 的分類 id。用 id 不用名稱——之後在 GitHub 上把分類
# 改成中文名也不會壞,giscus 實際比對的是 id。
GENERAL_CAT = CFG['site'].get('giscus', {}).get('category_general', '')
...
def main():
    ...
    splice('sw.js', 'ver',
           f"const SHELL = '{PREFIX}-shell-{EPOCH}-{digest(shell_files)}';\n"
           f"const ASSET = '{PREFIX}-asset-{EPOCH}-{digest(w[2:] for w in warm)}';")
